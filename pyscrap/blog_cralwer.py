...
from blog_cralwer_ui import * 

app_id = "f_dktXJac8EU37EU0daF"
secret_id = "jwZNcDgC5Y"
...
def main(keyword):
    ...
    sNode = 'blog'
    search_text = keyword # blog_crawler.py에서 keyword값을 받는다. 
    ...
    show_output(search_text, sNode)


def show_output(search_text, sNode) :
    print(">show_output()")
    ...
    for idx in range(len(data)) :
        ...
        if data[idx]["pDate"] >= (date.today() - timedelta(2)).strftime('%Y%m%d') : # 이틀 전까지 작성된 포스팅만 보기
            ...
            #내일은 show_output의 결과물을 print가 아니라 return으로 반환해서 ui.py에 output을 전달하고 setText로 textBrowser에 출력한다. 
